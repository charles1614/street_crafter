#!/usr/bin/env python3
"""
Script to download Waymo validation dataset.
Downloads tfrecord files from 1.4.1 dataset and parsed data from 2.0.0 dataset for validation.
"""
...
def download_tfrecord_file(filename, target_dir, scene_id):
    ...
    tfrecord_url = f"gs://waymo_open_dataset_v_1_4_1/individual_files/validation/{filename}.tfrecord"
...
def download_parsed_data(scene_id, target_dir):
    ...
    # Data types to download (same as training script)
    data_types = [
        ("lidar", "lidar"),
    ...
    for data_type, folder_name in data_types:
        ...
        parquet_url = f"gs://waymo_open_dataset_v_2_0_0/validation/{data_type}/{parquet_file}"
...
def download_validation_data(
    scene_ids: List[int],
    target_dir: str,
    waymo_val_list_path: str = "./waymo_val_list.txt"
) -> None:
    """
    Downloads validation data for specified scene IDs.
    
    Args:
        scene_ids (List[int]): List of scene IDs to download
        target_dir (str): Target directory to save downloaded files
        waymo_val_list_path (str): Path to waymo_val_list.txt file
    """
    # Create target directory
    target_path = Path(target_dir)
    target_path.mkdir(parents=True, exist_ok=True)
    
    # Read waymo validation list
    if not os.path.exists(waymo_val_list_path):
        print(f"Error: waymo_val_list.txt not found at {waymo_val_list_path}")
        sys.exit(1)
    
    with open(waymo_val_list_path, "r") as f:
        total_list = f.readlines()
    
    # Get filenames for the specified scene IDs
    file_names = [total_list[i].strip() for i in scene_ids]
    
    print(f"Downloading validation data for {len(scene_ids)} scenes...")
    ...
    print("\n✓ All validation data download completed!")
    print(f"Validation data location: {target_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Download Waymo validation dataset")
    parser.add_argument(
        "--target_dir",
        type=str,
        default="data/waymo/raw/validation",
    ...
    parser.add_argument(
        "--waymo_val_list",
        type=str,
        default="./waymo_val_list.txt",
        help="Path to waymo_val_list.txt file"
    )
    
    args = parser.parse_args()
    
    print("Note: `gcloud auth login` is required before running this script")
    print("Downloading Waymo validation dataset from Google Cloud Storage...")
    ...
    download_validation_data(scene_ids, args.target_dir, args.waymo_val_list)
